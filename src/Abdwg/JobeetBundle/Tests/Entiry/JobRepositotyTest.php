 ...
 * Time: 上午 10:25
 */
// src\Abdwg\JobeetBundle\Tests\Entiry\JobRepositotyTest.php
namespace Abdwg\JobeetBundle\Tests\Repository;
...
class JobRepositoryTest extends WebTestCase
{
    ...
    public function setUp()
    {
        ...
        $loader->loadFromDirectory(static::$kernel->locateResource('@AbdwgJobeetBundle/DataFixtures/ORM'));
    ...
    {
        $query = $this->em->createQuery('SELECT c FROM AbdwgJobeetBundle:Category c');
        $categories = $query->getResult();

        foreach($categories as $category) {
            $query = $this->em->createQuery('SELECT COUNT(j.id) FROM AbdwgJobeetBundle:Job j WHERE j.category = :category AND j.expires_at > :date');
            $query->setParameter('category', $category->getId());
            $query->setParameter('date', date('Y-m-d H:i:s', time()));
            $jobs_db = $query->getSingleScalarResult();

            $jobs_rep = $this->em->getRepository('AbdwgJobeetBundle:Job')->countActiveJobs($category->getId());
    ...
    public function testGetActiveJobs()
    {
        ...
        foreach ($categories as $category) {
            $query = $this->em->createQuery('SELECT COUNT(j.id) from AbdwgJobeetBundle:Job j WHERE j.expires_at > :date AND j.category = :category');
            $query->setParameter('date', date('Y-m-d H:i:s', time()));
            $query->setParameter('category', $category->getId());
            $jobs_db = $query->getSingleScalarResult();

            if($jobs_db > 2 ) {
                $jobs_rep = $this->em->getRepository('AbdwgJobeetBundle:Job')->getActiveJobs($category->getId(), 2);
                ...
                $jobs_rep = $this->em->getRepository('AbdwgJobeetBundle:Job')->getActiveJobs($category->getId(), 2, 1);
                // We set the limit to 2 results, starting from the second job and test if the result is as expected
                $this->assertEquals(2, count($jobs_rep));
            }

            $jobs_rep = $this->em->getRepository('AbdwgJobeetBundle:Job')->getActiveJobs($category->getId(), null, null);
    ...
    {
        $query = $this->em->createQuery('SELECT j FROM AbdwgJobeetBundle:Job j WHERE j.expires_at > :date');
        $query->setParameter('date', date('Y-m-d H:i:s', time()));
        $query->setMaxResults(1);
        $job_db = $query->getSingleResult();

        $job_rep = $this->em->getRepository('AbdwgJobeetBundle:Job')->getActiveJob($job_db->getId());
        // If the job is active, the getActiveJob() method should return a non-null value
        $this->assertNotNull($job_rep);

        $query = $this->em->createQuery('SELECT j FROM AbdwgJobeetBundle:Job j WHERE j.expires_at < :date');         $query->setParameter('date', date('Y-m-d H:i:s', time()));
        $query->setMaxResults(1);
        $job_expired = $query->getSingleResult();

        $job_rep = $this->em->getRepository('AbdwgJobeetBundle:Job')->getActiveJob($job_expired->getId());
    ...
    public function testGetForLuceneQuery()
    {
        ...
        $jobs = $em->getRepository('AbdwgJobeetBundle:Job')->getForLuceneQuery('FOO6');
        ...
        $jobs = $em->getRepository('AbdwgJobeetBundle:Job')->getForLuceneQuery('position:FOO7');
