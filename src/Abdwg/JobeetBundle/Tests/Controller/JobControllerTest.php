 ...
 * Time: 上午 11:55
 */
// src\Abdwg\JobeetBundle\Tests\Controller\JobControllerTest.php
...
class JobControllerTest extends WebTestCase
{
    ...
    public function getMostRecentProgrammingJob()
    {
        ...
        $query = $em->createQuery('SELECT j from AbdwgJobeetBundle:Job j LEFT JOIN j.category c WHERE c.slug = :slug AND j.expires_at > :date ORDER BY j.created_at DESC');
    ...
    public function getExpiredJob()
    {
        ...
        $query = $em->createQuery('SELECT j from AbdwgJobeetBundle:Job j WHERE j.expires_at < :date');
        $query->setParameter('date', date('Y-m-d H:i:s', time()));
        $query->setMaxResults(1);
    ...
    public function testIndex()
    {
        ...
        $this->assertEquals('Abdwg\JobeetBundle\Controller\JobController::showAction', $client->getRequest()->attributes->get('_controller'));
    ...
    public function testJobForm()
    {
        ...
        $form = $crawler->selectButton('Preview your job')->form(array(
                ...
                'job[file]'         => __DIR__.'/../../../../../web/bundles/abdwgjobeet/images/sensio-labs.gif',
        ...
        $client->submit($form);
        $this->assertEquals('Abdwg\JobeetBundle\Controller\JobController::createAction', $client->getRequest()->attributes->get('_controller'));

        //用户会被重定向到preview页面
        $client->followRedirect();
        $this->assertEquals('Abdwg\JobeetBundle\Controller\JobController::previewAction', $client->getRequest()->attributes->get('_controller'));

        // 测试数据库中是否有刚才创建的Job数据，还需要测试当用户未公布Job信息时is_activated列是否被设置成false
        $kernel = static::createKernel();
        $kernel->boot();
        $em = $kernel->getContainer()->get('doctrine.orm.entity_manager');
        $query = $em->createQuery('SELECT count(j.id) from AbdwgJobeetBundle:Job j WHERE j.location = :location AND j.is_activated IS NULL AND j.is_public = 0');
    ...
    public function testPublishJob()
    {
        ...
        $query = $em->createQuery('SELECT count(j.id) from AbdwgJobeetBundle:Job j WHERE j.position = :position AND j.is_activated = 1');
    ...
    public function testDeleteJob()
    {
        ...
        $query = $em->createQuery('SELECT count(j.id) from AbdwgJobeetBundle:Job j WHERE j.position = :position');
    ...
    public function testExtendJob()
    {
        ...
        $kernel->boot();
        $em = $kernel->getContainer()->get('doctrine.orm.entity_manager');
        $job = $em->getRepository('AbdwgJobeetBundle:Job')->findOneByPosition('FOO5');
    ...
    public function getJobByPosition($position)
    {
        ...
        $query = $em->createQuery('SELECT j from AbdwgJobeetBundle:Job j WHERE j.position = :position');
